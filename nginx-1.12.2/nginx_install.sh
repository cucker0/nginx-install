...
# 软件包
#  LuaJIT-2.0.5.tar.gz nginx-1.12.2.tar.gz   ngx_devel_kit-0.3.0.tag.gz   openssl-1.0.2o.tar.gz lua-nginx-module-0.10.9rc8.tar.gz  nginx_upstream_check_module-v1.12.1+.tar.gz  ngx_dynamic_upstream-0.1.6.tar.gz  pcre-8.42.tar.gz init.d.nginx nginx-1.12.2_install.sh
...
# 配置nginx
./configure --prefix=/usr/local/nginx_${nginx_version} --user=nginx --group=nginx --with-http_stub_status_module --with-http_ssl_module --with-pcre=$workdir/pcre-8.42 --with-http_realip_module --with-http_image_filter_module --with-http_gzip_static_module --with-openssl=$workdir/openssl-1.0.2o --with-openssl-opt=enable-tlsext --add-module=$workdir/ngx_devel_kit-0.3.0 --add-module=$workdir/lua-nginx-module-0.10.12 --add-module=$workdir/nginx_upstream_check_module-v1.12.1+ --add-module=$workdir/ngx_dynamic_upstream-0.1.6 --with-stream --with-stream_ssl_module
if [ $? !=0 ]; then
